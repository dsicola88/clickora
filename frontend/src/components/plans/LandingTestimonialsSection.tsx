...
export function LandingTestimonialsSection({ data, salesDark, textStyles: st, className }: Props) {
  ...
  const items =
    data.items?.filter(
      (it) =>
        Boolean(it.quote?.trim()) || (Boolean(it.thumbnail_url.trim()) && Boolean(it.video_url.trim())),
    ) ?? [];
  ...
  const quoteItems = items.filter((it) => it.quote?.trim());
  const videoItems = items.filter((it) => it.thumbnail_url.trim() && it.video_url.trim());
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3 md:gap-5">
          {quoteItems.map((item, index) => (
            <figure
              key={`q-${index}`}
              className={cn(
                "relative flex h-full flex-col rounded-2xl border p-5 md:p-6",
                salesDark
            ...
            >
              <Quote
                className={cn(
                  "mb-3 h-5 w-5 shrink-0 opacity-70",
                  salesDark ? "text-emerald-300" : "text-primary",
                )}
                aria-hidden
              />
              <blockquote
                className={cn(
                  "flex-1 text-sm leading-relaxed md:text-[0.95rem]",
                  salesDark ? "text-white/90" : "text-foreground/90",
                )}
              >
                {item.quote?.trim()}
              </blockquote>
              {(item.name?.trim() || item.role?.trim()) && (
                <figcaption className="mt-4 space-y-0.5 border-t border-white/10 pt-3">
                  {item.name?.trim() ? (
                    <p className={cn("text-sm font-semibold", salesDark ? "text-white" : "text-foreground")}>
                      {item.name.trim()}
                    </p>
                  ) : null}
                  {item.role?.trim() ? (
                    <p className={cn("text-xs", salesDark ? "text-white/55" : "text-muted-foreground")}>
                      {item.role.trim()}
                    </p>
                  ) : null}
                </figcaption>
            ...
            return (
              ...
              >
                ...
                <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <span className="flex h-14 w-14 items-center justify-center rounded-full bg-white/95 text-black shadow-lg transition group-hover:scale-110">
