...
export function getPaidMetaAssetsRoot(): string {
  return path.join(process.cwd(), "uploads", "paid-meta-assets");
}
...
export const paidMetaAssetUpload = multer({
  storage: multer.diskStorage({
    destination: (req: Request, _file, cb) => {
      ...
      const dir = path.join(getPaidMetaAssetsRoot(), projectId);
      ensureDir(dir);
      cb(null, dir);
    },
