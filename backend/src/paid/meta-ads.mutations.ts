 ...
 */
import type {
  PaidAdsChangeRequestType as ChangeRequestType,
...
export async function applyMetaAdsetDailyBudget(
  ...
  await prisma.paidAdsMetaAdset.update({
    where: { id: ag.id },
...
export async function applyMetaPauseEntity(
  ...
  if (p.level === "campaign") {
    ...
    const a = await prisma.paidAdsMetaAdset.findFirst({
    ...
    if (ext) {
      ...
      await prisma.paidAdsMetaAdset.update({ where: { id: a.id }, data: { status: "paused" } });
    }
  } else {
    const ad = await prisma.paidAdsMetaAd.findFirst({
    ...
    if (ext) {
      ...
      await prisma.paidAdsMetaAd.update({ where: { id: ad.id }, data: { status: "paused" } });
    }
  }
...
export async function applyMetaPublishCreative(
  ...
  const cr = await prisma.paidAdsMetaCreative.findFirst({
  ...
  const conn = await prisma.paidAdsMetaConnection.findFirst({ where: { projectId } });
  ...
  if (!creativeId) {
    ...
    creativeId = cRes.id;
    await prisma.paidAdsMetaCreative.update({
      where: { id: cr.id },
  ...
  if (localAd && !localAd.externalAdId) {
    ...
    if (aRes.id) {
      await prisma.paidAdsMetaAd.update({
        where: { id: localAd.id },
